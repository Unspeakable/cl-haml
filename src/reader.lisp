...
(defun down-indent (level tag-stack offset-stack in-filter-p)
  (when (plusp level)
    (dotimes (i level)
      (pop tag-stack)
    ...
    (when in-filter-p
      (setf in-filter-p nil)))
  (values tag-stack offset-stack in-filter-p))

(defun in-filter-text (line tag-stack offset-stack)
  (concatenate 'string
               "\\"
               (subseq line (* (- (length tag-stack )
                                  (length offset-stack)
                                  1)
                               2))))
          ...
          :unless (skip-p line)
            :do (loop :while (char= (char line (1- (length line))) #\\)
                ...
                (ppcre:register-groups-bind (blank rest)
                  ...
                  (multiple-value-setq (*tag-stack* *offset-stack* *in-filter*)
                    (down-indent
                       (indent-diff blank)
                       *tag-stack*
                       *offset-stack*
...
(defun @symbols (lst)
  (remove-if-not
    (lambda (x)
      (when (symbolp x)
        (let ((1st-char (char (symbol-name x) 0)))
          (or (char= #\@ 1st-char)
              (char= #\$ 1st-char)))))
